...
const useVerses = (st) => {
  ...
  useEffect(()=>{
    ...
    console.log(range)
    if(range){
      for(var i = skeys.indexOf(rangeVal.startingChapter);i <= skeys.indexOf(rangeVal.endingChapter);i++){
        console.log(runs + skeys.indexOf(rangeVal.startingChapter),skeys.indexOf(rangeVal.endingChapter)+1)
        console.log(rangeVal)
        if(runs === 0){
          start = rangeVal.startingVerse - 1
        }
        if(runs + skeys.indexOf(rangeVal.startingChapter)+1 === skeys.indexOf(rangeVal.endingChapter)+1){//change this 
          end = rangeVal.endingVerse - 1
        } else {
